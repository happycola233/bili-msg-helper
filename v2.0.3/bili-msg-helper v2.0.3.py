...
选择一项：
    ...
    ''')
            while True: # 一直循环
                用户选择 = input("选择Chanify还是QQ？（1代表Chanify，2代表QQ）[1/2] ")
                if 用户选择 == "1" or 用户选择 == "2": # 用户的选择有效
                    break # 跳出循环
                print ("请输入数字：1或2！") # 提示用户的选择无效，并再次循环
        ...
        配置内容 = '''\
{
    ...
    def main (): # 主程序
        ...
        用户登录数据 = eval(requests.get ("https://api.bilibili.com/x/web-interface/nav", headers = headers, verify=False).text) # 获取当前用户登录信息
        ...
        if QQ_启用 == "是":
            print("\n程序即将开始运行，请尽量最小化置顶窗口，以防止无法使QQ窗口置顶在最前端！")

        时间戳_短 = eval(requests.get("http://api.bilibili.com/x/report/click/now",headers=headers,verify=False).text).get("data").get("now")
        ...
        while True: # 无条件一直循环
            try:
                消息列表数据 = eval(requests.get("https://api.vc.bilibili.com/session_svr/v1/session_svr/new_sessions?begin_ts=" + 时间戳_长 + "&build=0&mobi_app=web",headers=headers,verify=False).text) # 获取消息列表

                if 消息列表数据.get("code") != 0: # 如果“消息列表数据”的状态码不是“0”，报错
                    print("[错误] " + 消息列表数据.get("message"))
                    if Chanify_启用 == "是":
                        requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,data={'title':"程序发生异常，请检查程序运行状态！",'text':"[错误] " + 消息列表数据.get("message"),'sound':1},verify=False).text                                        
                    if QQ_启用 == "是":
                        QQmsgsend(QQ_name,("程序发生异常，请检查程序运行状态！\n[错误] " + 消息列表数据.get("message")).encode('gbk', 'replace').decode('gbk'))

                    if 消息列表数据.get("code") == -412:
                        print("注：由于api访问频繁，程序将于15分钟后再试一次！")
                        time.sleep(60*15)

                if 用户登录数据.get("code") != 0: # 如果“用户登录数据”的状态码不是“0”，报错
                    print("[错误] " + 用户登录数据.get("message"))
                    if Chanify_启用 == "是":
                        requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,data={'title':"程序发生异常，请检查程序运行状态！",'text':"[错误] " + 用户登录数据.get("message"),'sound':1},verify=False).text                                        
                    if QQ_启用 == "是":
                    ...
                    if 消息列表数据.get("data").get("session_list") != None: # 如果有新消息
                        for i in range(len(消息列表数据.get("data").get("session_list"))):
                            ...
                            if 私信聊天数据.get("code") != 0: # 如果状态码不是“0”，报错
                                ...
                                if Chanify_启用 == "是":
                                    requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,data={'title':"程序发生异常，请检查程序运行状态！",'text':"[错误] " + 私信聊天数据.get("message"),'sound':1},verify=False).text                                        
                                if QQ_启用 == "是":
                                ...
                                for num in range(新消息数 -1,-1,-1):
                                    ...
                                    sender_info = eval(requests.get("https://api.bilibili.com/x/space/acc/info?mid=" + sender_uid,headers=headers,verify=False).text)
                                    if sender_info.get("code") != 0: # 如果“sender_info”的状态码不是“0”，报错
                                        print("[错误] " + sender_info.get("message"))
                                        if Chanify_启用 == "是":
                                            requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,data={'title':"程序发生异常，请检查程序运行状态！",'text':"[错误] " + sender_info.get("message"),'sound':1},verify=False).text                                        
                                        if QQ_启用 == "是":
    ...
    while True: # 一直循环
        if isConnected () == true: # 网络是否连接正常
            ...
            input ("无法连接到互联网，这会使任务无法完成！请检查网络是否正常！（按Enter键重试）")
