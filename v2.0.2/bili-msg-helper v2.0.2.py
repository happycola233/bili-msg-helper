...
try:
    ...
    def QQmsgsend(name,text):
        ...
        try:
            ...
            win32gui.PostMessage(win32gui.FindWindow("TXGuiFoundation", name), win32con.WM_CLOSE, 0, 0)
            if win32gui.FindWindow("TXGuiFoundation", name) == 0:
                print("[错误]无效的窗口句柄：昵称（有备注则使用备注）填写错误！")
        except:
            print("[错误]无效的窗口句柄：昵称（有备注则使用备注）填写错误！")
    ...
    2、使用QQ（支持安卓+苹果）
    说明：使用模拟点击方式发送消息，无需输入账号密码
    要求：需确保电脑上已安装电脑版QQ，并登录
    ''')
            while True: # 一直循环
    ...
    def main (): # 主程序
        ...
        if QQ_启用 == "是":
            print("\n程序即将开始运行，请打开QQ后关闭所有QQ窗口，使其最小化运行在托盘")
